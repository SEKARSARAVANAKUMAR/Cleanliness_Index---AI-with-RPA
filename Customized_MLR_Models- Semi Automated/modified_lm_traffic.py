...
a = None
try:
    query1="select * from sample_client_details where clientid=17;"# where clientid ="+s_cn+";"
...
for index,rows in z.iterrows():
    ...
    try:
        ...
        query3="select * from feature_dim where feature_name='traffic';"
    ...
    try:
        ...
        #training set
        train_set = agg_tra[(agg_tra['date_time'] > '2019-05-01 00:00:00') & (agg_tra['date_time'] <= '2019-08-31 23:30:00')]
        train_set.reset_index(inplace=True)
        trainning_set,testing_set=Model_Module_batch.Model_Data_Formation_Traffic(train_set)
        model_lm2,feature_sel,best_parameter,model_lm1=Model_Module_batch.Model_Building_Traffic(trainning_set)
        Mape,Mape_R=Model_Module_batch.Model_Evaluation_Testing_Traffic(best_parameter,model_lm2,testing_set,trainning_set)
        ...
        print("check the data formation")
    break
db.close()
dbe.close()